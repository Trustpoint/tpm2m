...
/**
 * Represents the permitted usages (as defined in the M2M specification) for a key pair. See
 * RFC 5280 section 4.2.1.3 for more details regarding the values here.
 */
public class KeyUsage {
    /**
     * The digitalSignature bit is asserted when the subject public key is used for verifying
     * digital signatures, other than signatures on certificates (bit 5) and CRLs (bit 6), such as
     * those used in an entity authentication service, a data origin authentication service, and/or
    ...
    /**
     * The keyEncipherment bit is asserted when the subject public key is used for enciphering
     * private or secret keys, i.e., for key transport. For example, this bit shall be set when an
     * RSA public key is to be used for encrypting a symmetric content-decryption key or an
    ...
    /**
     * The keyCertSign bit is asserted when the subject public key is used for verifying signatures
     * on public key certificates. If the keyCertSign bit is asserted, then the cA bit in the basic
    ...
    public String toString(int depth) {
        ...
        FormattingUtils.indent(buffer, depth).append(
                "KeyUsage BIT STRING {").append(LINE_SEPARATOR);

        if (getDigitalSignature()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "digitalSignature,").append(LINE_SEPARATOR);
        }

        if (getNonRepudiation()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "nonRepudiation,").append(LINE_SEPARATOR);
        }

        if (getKeyEncipherment()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "keyEncipherment,").append(LINE_SEPARATOR);
        }

        if (getDataEncipherment()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "dataEncipherment,").append(LINE_SEPARATOR);
        }

        if (getKeyAgreement()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "keyAgreement,").append(LINE_SEPARATOR);
        }

        if (getKeyCertSign()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "keyCertSign,").append(LINE_SEPARATOR);
        }

        if (getCrlSign()) {
            FormattingUtils.indent(buffer, depth + 1).append(
                    "cRLSign,").append(LINE_SEPARATOR);
        }
    ...
    private static boolean isValidKeyUsageByte(byte keyUsage) {
        ...
        return ((keyUsage &
                ((byte) ~(DIGITAL_SIGNATURE | NON_REPUDIATION | KEY_ENCIPHERMENT |
