...
/**
 * This class implements the M2M validation algorithm for certification paths consisting
 * exclusively of {@link M2mCertificate} objects. It uses the specified input parameter set (which
 * must be a {@link M2mParameters} object).
 */
public final class M2mCertPathValidator extends CertPathValidatorSpi {
    /**
     * Validates a certification path consisting exclusively of <code>M2MCertificate</code>s using
     * the M2M validation algorithm, which uses the specified input parameter set. The input
     * parameter set must be a {@link M2mCertPathParameters} object.
     *
    ...
    public CertPathValidatorResult engineValidate(CertPath certPath, CertPathParameters params)
        ...
        if (certPath == null) {
            ...
            throw new InvalidAlgorithmParameterException(
                "certPath must be an instance of M2mCertPath.");
        ...
        if (params != null) {
            ...
            validityDate =
                (certPathParameters.getValidityDate() != null) ?
        ...
        for (int i = certificateChain.size() - 1; i >= 0; i--) {
                ...
                if (allowSelfSignedRoot && (certificate.getSubject() != null) &&
                    ...
                    for (M2mTrustAnchor anchor : anchors) {
                        if (certificate.getIssuer().equals(anchor.getCaName())) {
                            ...
                            if (anchor.getCertificate() != null) {
                                ...
                                // Create a dummy certificate for the trust anchor so the rest of
                    ...
                    if (trustAnchor == null) {
                        throw new CertPathValidatorException(
                            "Unable to verify root of this chain.");
                    }
                } else {
            ...
            try {
                ...
                throw new CertPathValidatorException(
                    "Certificate expired or not yet valid.", ex, certPath, i);
            }
             ...
             *     verification but may be omitted from the transmitted certificate and filled in
             *     from the pkAlgorithm of the superior certificate (but not root certificate)
             *
             *
             * cAAlgParams OCTET STRING OPTIONAL
             *     Required for signature verification unless absent in both the transmitted
             *     certificate and the superior certificate pKAlgParams field. Fill in from
             *     superior certificate pKAlgParams field if needed (but not root certificate)
             */
             ...
             *     Required for signature verification but may be omitted from the transmitted
             *     certificate and filled in from the subject field of the superior cert (but
            ...
            try {
                ...
                throw new CertPathValidatorException(
                    "Signature verification failed.", ex, certPath, i);
            }
