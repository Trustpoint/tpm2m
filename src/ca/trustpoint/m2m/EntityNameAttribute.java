...
/**
 * Represents an attribute of an EntityName object. The M2M spec defines these attributes as one
 * of:
 *
 * <pre>
...
public class EntityNameAttribute {
    ...
    /**
     * Returns an ASN1Encodable object of this instance value.
     *
     * @return An ASN1Encodable object of this instance value.
     * @throws IOException if this instance is invalid.
     */
    public ASN1Encodable getEncodedValue() throws IOException {
        if (!isValid()) {
    ...
    public byte[] getEncoded() throws IOException {
        return new DERTaggedObject(false, id.getIndexId(), getEncodedValue()).getEncoded();
    }

    /**
     * Return true if this instance is a valid EntityName attribute. Values are checked for length
     * and allowed content against the attribute type rules.
     *
        ...
        switch (id) {
            case Country: // 0.
                FormattingUtils.indent(buffer, depth).append(
                        "[0] country PrintableString: ");
                break;

            case Organization: // 1.
                FormattingUtils.indent(buffer, depth).append(
                        "[1] organization UTF8String: ");
                break;

            case OrganizationalUnit: // 2.
                FormattingUtils.indent(buffer, depth).append(
                        "[2] organizationalUnit UTF8String: ");
                break;

            case DistinguishedNameQualifier: // 3.
                FormattingUtils.indent(buffer, depth).append(
                        "[3] distinguishedNameQualifier PrintableString: ");
                break;

            case StateOrProvince: // 4.
                FormattingUtils.indent(buffer, depth).append(
                        "[4] stateOrProvince UTF8String: ");
                break;

            case Locality: // 5.
                FormattingUtils.indent(buffer, depth).append(
                        "[5] locality UTF8String: ");
                break;

            case CommonName: // 6.
                FormattingUtils.indent(buffer, depth).append(
                        "[6] commonName UTF8String: ");
                break;

            case SerialNumber: // 7.
                FormattingUtils.indent(buffer, depth).append(
                        "[7] serialNumber PrintableString: ");
                break;

            case DomainComponent: // 8.
                FormattingUtils.indent(buffer, depth).append(
                        "[8] domainComponent IA5String: ");
                break;

            case RegisteredId: // 9.
                FormattingUtils.indent(buffer, depth).append(
                        "[9] registeredId OBJECT IDENTIFIER: ");
                break;

            case OctetsName: // 10.
                FormattingUtils.indent(buffer, depth).append(
                        "[10] octetsName OCTET STRING: ");
                break;
